...
static ssize_t gear4_enable_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct ufs_hba *hba = dev_get_drvdata(dev);
...
static ssize_t gear4_enable_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct ufs_hba *hba = dev_get_drvdata(dev);
	unsigned long flags, value;
	...
	hba->gear4_enable = value;

	spin_unlock_irqrestore(hba->host->host_lock, flags);
	return count;
}

static ssize_t gear4_threshold_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct ufs_hba *hba = dev_get_drvdata(dev);
...
static ssize_t gear4_threshold_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct ufs_hba *hba = dev_get_drvdata(dev);
	unsigned long flags, value;
	...
	hba->gear4_threshold = value;

	spin_unlock_irqrestore(hba->host->host_lock, flags);
	return count;
}

static ssize_t gear4_time_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct ufs_hba *hba = dev_get_drvdata(dev);
...
static ssize_t gear4_time_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct ufs_hba *hba = dev_get_drvdata(dev);
	unsigned long flags, value;
	...
	hba->gear4_time = value;

	spin_unlock_irqrestore(hba->host->host_lock, flags);
	return count;
}

static ssize_t force_gear5_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct ufs_hba *hba = dev_get_drvdata(dev);
...
static ssize_t force_gear5_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct ufs_hba *hba = dev_get_drvdata(dev);
	unsigned long flags, value;
	...
	hba->force_gear5 = value;

	spin_unlock_irqrestore(hba->host->host_lock, flags);
	return count;
}

static ssize_t gear5_threshold_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct ufs_hba *hba = dev_get_drvdata(dev);
...
static ssize_t gear5_threshold_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct ufs_hba *hba = dev_get_drvdata(dev);
	unsigned long flags, value;
...
static DEVICE_ATTR_RW(gear4_enable);
static DEVICE_ATTR_RW(gear4_threshold);
static DEVICE_ATTR_RW(gear4_time);
static DEVICE_ATTR_RW(force_gear5);
static DEVICE_ATTR_RW(gear5_threshold);
...
static struct attribute *ufs_sysfs_ufshcd_attrs[] = {
	...
	&dev_attr_wb_on.attr,
	&dev_attr_gear4_enable.attr,
	&dev_attr_gear4_threshold.attr,
	&dev_attr_gear4_time.attr,
	&dev_attr_force_gear5.attr,
	&dev_attr_gear5_threshold.attr,
	NULL
};
...
static inline bool ufshcd_is_hpb_flags(enum flag_idn idn)
{
	return ((idn == QUERY_FLAG_IDN_HPB_RESET) &&
		(idn <= QUERY_FLAG_IDN_HPB_EN));
}
...
static ssize_t wb_enable_show(struct device *dev,
				       struct device_attribute *attr, char *buf)
{
	bool flag;
	u8 index = 0;
...
static ssize_t wb_enable_store(struct device *dev,
				  struct device_attribute *attr,
				  const char *buf, size_t count)
{
	int ret;
	...
	dev_err(hba->dev, "%s write booster %s %d\n",
			__func__, enable ? "enable" : "disable", ret);
...
static ssize_t wb_flush_en_show(struct device *dev,
				       struct device_attribute *attr, char *buf)
{
	bool flag;
	u8 index = 0;
...
static ssize_t wb_flush_en_store(struct device *dev,
				  struct device_attribute *attr,
				  const char *buf, size_t count)
{
	int ret;
	...
	if (!(enable ^ hba->dev_info.wb_buf_flush_enabled))
		goto out;
	...
	dev_dbg(hba->dev, "%s write booster flush %s %d\n",
			__func__, enable ? "enable" : "disable", ret);
...
static ssize_t wb_flush_during_h8_show(struct device *dev,
				       struct device_attribute *attr, char *buf)
{
	bool flag;
	u8 index = 0;
...
static ssize_t wb_flush_during_h8_store(struct device *dev,
				  struct device_attribute *attr,
				  const char *buf, size_t count)
{
	int ret;
	...
	if (!(enable ^ hba->dev_info.wb_flush_during_h8))
		goto out;
	...
	index = ufshcd_wb_get_query_index(hba);
	ret = ufshcd_query_flag_retry(hba, opcode,
				      QUERY_FLAG_IDN_WB_BUFF_FLUSH_DURING_HIBERN8, index, NULL);
	if (ret) {
		dev_err(hba->dev, "%s write booster %s failed %d\n",
			__func__, enable ? "enable" : "disable", ret);
		return -EINVAL;
	...
	hba->dev_info.wb_flush_during_h8 = enable;
	dev_dbg(hba->dev, "%s write booster flush %s %d\n",
			__func__, enable ? "enable" : "disable", ret);
				       ...
				       struct device_attribute *attr, char *buf)
{
	...
	int ret;
	struct ufs_hba *hba = dev_get_drvdata(dev);
	if (ufshcd_is_hpb_flags(QUERY_FLAG_IDN_HPB_EN))
		index = ufshcd_wb_get_query_index(hba);
	pm_runtime_get_sync(hba->dev);
	ret = ufshcd_query_flag(hba, UPIU_QUERY_OPCODE_READ_FLAG,
		QUERY_FLAG_IDN_HPB_EN, index, &flag);
	pm_runtime_put_sync(hba->dev);
	if (ret)
		return -EINVAL;
	return sysfs_emit(buf, "%s\n", flag ? "true" : "false");
				  ...
				  const char *buf, size_t count)
{
	...
	if (!(enable ^ hba->dev_info.hpb_enabled))
		goto out;
	...
	index = ufshcd_wb_get_query_index(hba);
	ret = ufshcd_query_flag_retry(hba, opcode,
				      QUERY_FLAG_IDN_HPB_EN, index, NULL);
	if (ret) {
		dev_err(hba->dev, "%s write booster %s failed %d\n",
			__func__, enable ? "enable" : "disable", ret);
		return -EINVAL;
	...
	hba->dev_info.hpb_enabled = enable;
	dev_dbg(hba->dev, "%s write booster flush %s %d\n",
			__func__, enable ? "enable" : "disable", ret);
...
DEVICE_ATTR_RW(wb_enable);
DEVICE_ATTR_RW(wb_flush_en);
DEVICE_ATTR_RW(wb_flush_during_h8);
DEVICE_ATTR_RW(hpb_enable);
				  ...
				  const char *buf, size_t count)
{
	...
	ufshcd_wb_toggle_flush_during_h8(hba, (bool)value);
